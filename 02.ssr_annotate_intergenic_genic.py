...
def process_ssr_genic(ssr_file, chr_dic,outf):
    out_file = open(outf[1], 'w')
    inf = open(ssr_file,'r')
    genic_list = []
    for line in islice(inf, 0, None):
        gene_flag = 0
        ...
        if ssr_chr in  chr_dic:
            ...
            for gene_loc in gene_list[ssr_start_nr]:
                ...
                if ssr_start >= start and ssr_end <= end:
                    ...
                    break
        if gene_flag == 0:
            intergenic_line = f"{ssr_chr},{ssr_start},{ssr_end},{ssr},{ssr_type},{ssr_repeat},{ssr_len},Intergenic\n"
            out_file.write(intergenic_line)
...
def outf_name_fun(ssr_file,file_path):
    ...
    outf_intergenic_name = base_name + '_intergenic.csv'

    out_file = os.path.join(file_path, outf_name)
    outf_intergenic_file = os.path.join(file_path, outf_intergenic_name)
    outf = [out_file,outf_intergenic_file]
    return outf

def process_utr(genic_list,chr_dic,outf):
    out_file = open(outf[0],'w')
    for i in islice(range(len(genic_list)), 0, None):
        data = genic_list[i].strip().split(',')
        ssr_chr = data[0]
        ssr = data[1]
...
def main():
    ...
    outf = outf_name_fun(args.ssr_file, file_path)
    chr_dic = process_gff(args.gff_file)
    genic_list = process_ssr_genic(args.ssr_file, chr_dic,outf)
    genic_list = process_cds(genic_list, chr_dic)
    process_utr(genic_list, chr_dic, outf)
