...
def process_ssr_genic(ssr_file, gene_list,out_file):
    inf = open(ssr_file,'r')
    ...
    # for line in islice(inf, 1, None):
        ...
        for gene_loc in gene_list[ssr_start_nr]:
            ...
            if ssr_start >= start and ssr_end <= end:
                ...
                genic_line = f"{ssr_chr},{ssr},{ssr_type},{ssr_repeat},{ssr_start},{ssr_end},{ssr_len},exon\n"
...
def main():
    ...
    gene_list = process_gff(args.gff_file)
    process_ssr_genic(args.ssr_file, gene_list,out_file)
