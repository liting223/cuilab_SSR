...
def process_gff(gff_file):
    ...
    return gene_list,cds_list,utr_list

def process_ssr_genic(ssr_file, gene_list):
    inf = open(ssr_file,'r')
    genic_list = []
    for line in islice(inf, 0, None):
...
def process_cds(genic_list,cds_list):
...
def process_utr(genic_list,utr_list,out_file):
...
def main():
    ...
    gene_list,cds_list,utr_list = process_gff(args.gff_file)
    genic_list = process_ssr_genic(args.ssr_file, gene_list)
    genic_list = process_cds(genic_list, cds_list)
    out_file = outf_name_fun(args.ssr_file,file_path)
    process_utr(genic_list, utr_list, out_file)
